...
%In Chapter 2 the dated, as well as more recent literature is reviewed ...
%After that 


%In the following we will go through the systems components step by step and thereby delve into their implementation details and explain decisions made. The goal is to give an indepth overview of the system and also explain the theory behind and the inner workings of the models used.

%In the following, we will delve into the system’s architecture, providing implementation details, explaining the user interface and presenting the findings of a user study conducted to assess the system’s result quality.
...
%- as already mentioned the end goal of this project is to have a web application, where a user can input 

%I am writing a bachelors thesis where the goal is to have a webapp, where the user can input some query (eg. elon musk), then thousands of articles are downloaded from the internet through some api, preprocessed and split into sentences. those sentences are then fed into distilroberta which is finetuned to detect if the sentence has some future related content. then the future related sentences are clustered into topics with bertopic, and timetagged. the website features a timeline where the sentences that have temporal expressions in them are plotted and under the timeline is a word-cloud with all the topics with the sentences that have no temporal expressions inside them.

%- i have explained all models I used in this project (preprocessing, distilroberta, bertopic, timetagger)
%- now i have a chapter called "Frontend and Backend Implementation"
  %- there I want to go into detail how I structured the frontend and the backend and go into all the design choices and so on

%- Frontend: vue3 with apexcharts for the timeline and axios for http-requests
%- Backend: implemented with django, mysql and the classifier, clustering and tagging are run on a google colab instance due to necessary compute (django does a http request to a server in a running colab notebook)

%- could you please write this chapter out for me?
  %- it should span about 10 a4 pages
  %- also find a better heading for it
...
\bibliographystyle{unsrt}% the recommended bibstyle
